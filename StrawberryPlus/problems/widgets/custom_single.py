...
class myCustom_Single(OWWidget):
    name = "Custom Problem in Python - Single-Objective"
    description = "Write a Custom Single-Objective Problem in Python"
    ...
    class Inputs:
        ...
        problem = Input("Problem", CustomProblem, replaces=["in_problem"],
                        default=False, multiple=True)
    ...
    class Outputs:
        ...
        problem = Output("Problem", CustomProblem, replaces=["out_problem"])

    signal_names_in = ("data", "learner", "classifier", "object", "problem")
    signal_names_out = ("data", "learner", "classifier", "object", "problem")
    ...
    def __init__(self):
        ...
        for name in self.signal_names_out:
            setattr(self, name, {})
        ...
        gui.label(
            ...
            "<li>".join(map("in_{0}, in_{0}s".format, self.signal_names_in)) +
            "</ul></p><p>Output variables:<ul><li>" +
            "<li>".join(map("out_{0}".format, self.signal_names_out)) +
            "</ul></p>"
        )
    ...
    @Inputs.problem
    def set_object(self, data, sig_id):
        self.handle_input(data, sig_id, "problem")
    ...
    def initial_locals_state(self):
        ...
        for name in self.signal_names_in:
    ...
    def commit(self):
        ...
        for signal in self.signal_names_out:
            if signal == "problem":
                self.problem = self.console.locals.get("out_" + signal)
                ...
                print('enviado a ' + str(signal) + ' --> ' + str(out_var))

            # self.problem = str(self.text.toPlainText())
            # self.Outputs.problem.send(self.problem)
